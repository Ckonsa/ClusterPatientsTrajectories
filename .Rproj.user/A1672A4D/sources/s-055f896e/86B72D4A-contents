...
#   - k: integer, k > 0
# Output:
#   - vector containing k most frequent values in a column
...
#   - elements: vector
# Output:
#   - vector without multiple same elements in a row
# Example:
...
#   - n: integer n > 0
# Output:
#   - vector that is patients drug trajectory that contains drug codes.
...
# Input:
#   - string: string
#   - indexes: numeric vector
# Output:
#   - vector that contains pieces of the string
# Example:
...
# Output:
#   - numeric distance between two codes
# Example:
...
# Input:
#   - data: dataframe
#   - records_range: numeric vector with two values - min and max record count
# Output:
#   - vector that contains IDs of patients that fit into given range
...
# Input:
#   - patients_ids: numeric vector
#   - clusters_with_indexes: numeric vector
# Output:
#   - vector which has a list for every cluster.
#     Lists contain patients IDs, who belong to the cluster.
...
# Create a plot which has a plot for each cluster.
# Plots are showcasing occurrences of different drugs per patient.
# Input:
#   - data: dataframe
#   - clusters: vector of lists, every list represents one cluster with IDs
#   - drugs: string vector which contains drug codes
# Output:
#   - plot of drug occurrences in clusters
...
# Most frequent drug per patient is taken into account.
# Input:
#   - data: dataframe
#   - cluster: numeric vector which has patients IDs
# Output:
#   - string of most frequent drug code in given cluster
get_most_frequent_drug <- function(data, cluster) {
  frequent_drugs <- c()
  for (i in seq(along = cluster)) {
    patient <- cluster[i]
    filtered_data <- data %>% filter(SUBJECT_ID == patient)
    frequent_drug <- get_most_frequent_values(filtered_data, "DRUG_SOURCE_VALUE", 1)
...
# Gets values only from indexes in matrix.
# Expects for matrix to be symmetrical and removes duplicates.
# Input:
#   - matrix: matrix
#   - indexes: numeric vector
# Output:
#   - vector with matrix values where indexes intersect without duplicates
...
# Input:
#   - matrix: matrix
#   - max_cluster: integer n > 0, maximum number of clusters
# Output:
#   - vector with average standard deviations in clusters.
#     On index 1 is average SD with 1 cluster, on index 2 with 2 clusters and so on.
...
#   - matrix: matrix
# Output:
#   - plot which shows average standard deviation in clusters
#     if matrix is clustered into 1, 2, 3, ... clusters
...
#   - id: integer
# Output:
#   - dataframe which shows count of all different drugs written to patient
...
# Input:
#   - data: dataframe of drug codes with their frequencies
#   - drug: string
# Output:
#   - count of the drug in data, 0 if drug is not in data
...
# Input:
#   - data: dataframe
#   - clusters: vector with lists for every cluster
# Output:
#   - dataframe with additional data about every cluster
...
# Input:
#   - patient_data: list with patients IDs and trajectories
#   - punishments: numeric vector
#   - dtw: boolean if we want to use DTW algorithm
# Output:
#   - symmetrical matrix which values are distances between patients trajectories
...
# Converts binary to image file.
# Input:
#   - bin: binary code of image
#   - name: name of the file with suitable file extension (ex. graph_1.png)
# Output:
#   - image file is created with given name
